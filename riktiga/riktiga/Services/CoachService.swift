...
struct CoachProgram: Identifiable, Codable {
    ...
    let dailyTips: [String?]? // Tips för varje dag (index 0 = måndag, 6 = söndag)
...
struct ProgramRoutine: Identifiable, Codable {
    ...
    let scheduledDays: [Int]? // Vilka veckodagar rutinen är tilldelad (0 = måndag, 6 = söndag)
    
    private enum CodingKeys: String, CodingKey {
        case id, name, title, note, exercises
        case scheduledDays = "scheduled_days"
        case scheduledDaysCamel = "scheduledDays"
    }
    ...
    init(from decoder: Decoder) throws {
        ...
        // Hämta schemalagda dagar - prova både snake_case och camelCase
        if let days = try container.decodeIfPresent([Int].self, forKey: .scheduledDays) {
            scheduledDays = days
        } else if let days = try container.decodeIfPresent([Int].self, forKey: .scheduledDaysCamel) {
            scheduledDays = days
        } else {
            scheduledDays = nil
        }
